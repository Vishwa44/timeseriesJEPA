...
class PatachTST_embedding(nn.Module):
    ...
    def forward(self, x, masks=None):           # x: [Batch, Input length, Channel]
        x = x.permute(0,2,1)    # x: [Batch, Channel, Input length]
        x = self.model(x, masks)
        x = x.permute(0,1,3,2)
        return x
...
class TSTiEncoder(nn.Module):  #i means channel-independent
    ...
    def forward(self, x, masks) -> Tensor:                                              # x: [bs x nvars x patch_len x patch_num]
        ...
        x = self.dropout(x + self.W_pos)                                         # u: [bs * nvars x patch_num x d_model]
